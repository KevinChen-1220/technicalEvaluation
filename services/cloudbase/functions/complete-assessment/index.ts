import { completeAssessment, type AssessmentRepository } from '../../server/assessment/service';
...
export function createMain(dependencies: Dependencies) {
  return async (event: unknown, _context: unknown): Promise<unknown> => {
    try {
      return await completeAssessment(event, getTrustedWeChatContext(), dependencies);
    } catch (error) {
