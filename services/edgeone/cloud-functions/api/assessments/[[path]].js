...
// node-functions/api/assessments/[[path]].ts
var path_exports = {};
...
// ../../packages/assessment-core/src/scoring.ts
function scoreAssessment(paper, session) {
...
function optionSetsEqual(left, right) {
  return left.length === right.length && left.every((id, index) => id === right[index]);
}
function findLevel(levels, accuracy) {
...
function buildKnowledgePointResults(paper, questionResults) {
  ...
  return [...aggregate.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([knowledgePoint, value]) => ({
    knowledgePoint,
    total: value.total,
...
async function createAssessmentsRoute(request, context, injected) {
  try {
    ...
    if (segments.length === 0 && request.method === "GET") {
      ...
      const records = (await Promise.all(page.map(async (summary) => await dependencies.stores.assessments.get(identity.ownerKey, summary.id)))).filter((record) => record !== null);
      return success({
        type: "listed",
    ...
    if (assessmentId === void 0) throw invalidRequest();
    if (segments.length === 1 && request.method === "GET") {
      const record = await dependencies.stores.assessments.get(identity.ownerKey, assessmentId);
      return record === null ? success({ type: "not_found", errorCode: "INVALID_REQUEST" }) : success({ type: "found", assessment: toClientAssessment(record) });
    }
    if (segments.length === 1 && (request.method === "PUT" || request.method === "PATCH")) {
      ...
      if (current === null) return success({ type: "not_found", errorCode: "INVALID_REQUEST" });
      ...
      if (result.type === "conflict") {
        ...
        if (latest === null) return success({ type: "not_found", errorCode: "INVALID_REQUEST" });
        return success({ type: "conflict", current: toClientAssessment(latest) });
      }
      return success({ type: "updated", revision: result.record.revision });
    }
    ...
    if (segments.length === 2 && segments[1] === "complete" && request.method === "POST") {
      ...
      return success({ type: "completed", assessment: toClientAssessment(result.record) });
    }
...
function assessmentPath(rawUrl) {
...
function parseAnswers(value, paper) {
  ...
  for (const [questionId, selected] of Object.entries(value)) {
    ...
    if (question === void 0 || !Array.isArray(selected) || !selected.every((option) => typeof option === "string")) {
      throw invalidRequest();
    }
...
function assertCompleteAnswers(answers, paper) {
...
function toClientAssessment(record) {
  const base = {
    id: record.id,
...
function answerablePaper(paper) {
  return {
    id: paper.id,
    topic: paper.topic,
    ...
    questions: paper.questions.map((question) => ({
      ...
      ...question.materials === void 0 ? {} : { materials: answerableMaterials(question.materials) }
    }))
  };
}
function answerableMaterials(materials) {
    ...
    switch (material.type) {
      ...
      case "image":
        return {
          ...
          uri: material.uri,
          alt: material.alt,
          ...material.caption === void 0 ? {} : { caption: material.caption },
          ...material.aspectRatio === void 0 ? {} : { aspectRatio: material.aspectRatio }
        };
      case "table":
        return {
          type: material.type,
      ...
      case "bar_chart":
        return {
          type: material.type,
          ...material.title === void 0 ? {} : { title: material.title },
          ...material.unit === void 0 ? {} : { unit: material.unit },
          items: material.items.map((item) => ({
            label: item.label,
            value: item.value,
            ...item.displayValue === void 0 ? {} : { displayValue: item.displayValue }
          }))
        };
    }
  });
}
function toClientSummary(record) {
