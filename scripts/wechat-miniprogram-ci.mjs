...
const privateKeyPath = args['private-key-path'] ?? process.env.WECHAT_PRIVATE_KEY_PATH ?? '';
...
if (mode === 'dry-run') {
  ...
  process.stdout.write(JSON.stringify({
    ...
    privateKeyPath: privateKeyPath ? '<redacted>' : '<not-required-for-dry-run>',
    robot,
    version,
    description,
...
if (privateKeyPath.trim().length === 0 || !existsSync(privateKeyPath)) {
