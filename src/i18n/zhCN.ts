...
function localizeValidationDetails(details: string): string {
  return details
    ...
    .replace(/Question ([^ ]+) material (\d+) table caption is required\./g, '第 $1 题的第 $2 个表格资料缺少标题。')
    ...
    .replace(/Question ([^ ]+) material (\d+) bar_chart title is required\./g, '第 $1 题的第 $2 个柱状图资料缺少标题。')
    .replace(/Question ([^ ]+) material (\d+) bar_chart unit is required\./g, '第 $1 题的第 $2 个柱状图资料缺少单位。')
