...
- KV 与 Blob 的 1 GB 免费空间足够少量用户的会话、配置、任务和试卷历史。
...
- `src/storage`：KV/Blob 端口与 EdgeOne 适配器。
...
5. 服务端返回随机 session token；KV 只存 token 的 SHA-256、OpenID 哈希和过期时间。
...
- `session:<tokenHash>`：owner key、创建时间、过期时间。
- `settings:<ownerKey>`：语言、隐私同意版本和显示偏好。
- `quota:<ownerKey>:<yyyy-mm-dd>`：当日生成次数和最后请求时间。
- `job:<ownerKey>:<clientRequestId>`：生成状态、assessmentId、错误码和版本。
- `index:<ownerKey>`：按更新时间倒序的测评摘要，最多保留 200 条。
...
Blob/KV 可能最终一致。写入成功的响应直接返回已写对象，本地立即保存；后续列表或跨设备读取遇到短暂旧值时按 revision 重试。更新使用 revision 乐观锁，冲突返回 `REVISION_CONFLICT`。
...
- 每次生成前检查全局手动熔断键 `ops:generation-disabled`。
...
- 适配器测试：内存 KV/Blob 和 EdgeOne context 双实现行为一致。
