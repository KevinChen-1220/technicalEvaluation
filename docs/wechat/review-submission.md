...
本文件用于把本地发布候选提交到微信审核。它不声明审核已通过，也不填写真实主体、AppID、备案或生产 credentials。所有 `待配置` 都要由发布负责人从微信公众平台、CloudBase 控制台和法定材料中替换。
...
2. 填写 commit SHA、tag、WeChat 版本号、CloudBase 生产 env id、artifact hash、审核单链接、灰度比例和回滚版本。
...
如果 formal verifier 因 `待配置`、开发 env、fixture、内容安全占位值或包内 disclosure 不一致而失败，不能提交审核。仓库模板失败是正确行为；只有真实 production disclosure 和生产环境变量齐全时才会通过。
...
2. 手动 `workflow_dispatch`，`publish_target=upload`，填写版本、描述、robot 和生产 disclosure 文件路径。
...
```text
SkillScope 是一个技能测评小程序。用户输入主题和可选侧重点后，系统通过服务端模型生成 50/100 道单选、多选或判断题；用户答题后获得本地/服务端评分、知识点统计、错题复盘和题目解析。

首次进入会展示隐私保护指引。小程序不主动获取手机号、头像、昵称或通讯录；答题记录与设置按微信登录态隔离保存在 CloudBase。投诉入口：我的/隐私页面 -> 反馈。
...
- 隐私驳回：对照 `docs/wechat/privacy-data-map.md` 和微信后台隐私声明，确认 CloudBase、输入主题、答题记录、投诉反馈字段一致。
- AI 披露驳回：核对设置页/隐私页、生产 disclosure、manifest 和备案/登记材料是否一致。
- 内容安全驳回：查看 CloudBase 输入/输出审核日志，只记录错误码和 job id，不导出用户题目全文。
