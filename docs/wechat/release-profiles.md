...
- 模式边界：formal profile 只接受固定 `--profile formal` 与可选 `--disclosure-file`；明确拒绝 `--check-only`、`--file`、`--mode`、`--dist`、`--preflight-only`、重复参数和未知参数。`npm run verify:wechat-release:formal-preflight` 由独立脚本执行，只用于部署前检查配置，不代表 formal 发布验证成功，也不会产生同轮构建与产物证据。
